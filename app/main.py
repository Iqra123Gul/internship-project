...
from xai import compute_vit_attention_map

app = FastAPI(title="Flickr8k BLIP Captioning API")

MODEL_DIR = os.environ.get("MODEL_DIR", os.path.join(os.path.dirname(__file__), "..", "models", "week2_baseline_blip"))
...
async def caption_image(file: UploadFile = File(...), question: str = Form(None)):
    ...
    if question:
        ...
        response["note"] = "VQA not supported by this baseline model; caption-only response returned."
